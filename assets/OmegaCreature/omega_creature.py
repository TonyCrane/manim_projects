...
omega_creature_dir_maybe = os.path.join(MEDIA_DIR, "assets", "OmegaCreature")
if os.path.exists(omega_creature_dir_maybe):
    OMEGA_CREATURE_DIR = omega_creature_dir_maybe
else:
    OMEGA_CREATURE_DIR = os.path.join("assets")
...
class OmegaCreature(SVGMobject):
    CONFIG = {
        "color": BLUE_E,
        "file_name_prefix": "OmegaCreatures",
        "stroke_width": 0,
        "stroke_color": BLACK,
        "fill_opacity": 1.0,
        "height": 2,
    ...
    def make_eye_contact(self, omega_creature):
        self.look_at(omega_creature.eyes)
        omega_creature.look_at(self.eyes)
        return self
...
def get_all_omega_creature_modes():
    result = []
...
class Randolph(OmegaCreature):
    pass  # Nothing more than an alternative name


class Mortimer(OmegaCreature):
    CONFIG = {
        "color": GREY_BROWN,
        "flip_at_start": True,
    }
...
class Mathematician(OmegaCreature):
    CONFIG = {
        "color": GREY,
    }


class BabyPiCreature(OmegaCreature):
    CONFIG = {
        "scale_factor": 0.5,
    ...
    def __init__(self, *args, **kwargs):
        OmegaCreature.__init__(self, *args, **kwargs)
...
class TauCreature(OmegaCreature):
    CONFIG = {
        "file_name_prefix": "TauCreatures"
    }


class ThreeLeggedOmegaCreature(OmegaCreature):
    CONFIG = {
        "file_name_prefix": "ThreeLeggedOmegaCreatures"
    }
...
class Eyes(VMobject):
    ...
    def create_eyes(self, mode=None, thing_to_look_at=None):
        ...
        omega = OmegaCreature(mode=mode)
        eyes = VGroup(omega.eyes, omega.pupils)
        if self.submobjects:
