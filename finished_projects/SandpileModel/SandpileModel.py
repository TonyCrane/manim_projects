...
class Test(Scene):
    def construct(self):
        field = np.loadtxt("sand-100w.txt", dtype=int, delimiter=",")
        pixel = np.zeros((field.shape[0], field.shape[1], 4), dtype="uint8")
        colors = color_gradient([BLACK, GREEN, YELLOW], 4)
        for i in range(field.shape[0]):
            for j in range(field.shape[1]):
                pixel[i][j] = color_to_int_rgba(colors[field[i][j]])
        img = ImageMobject(pixel, height=8)
        self.add(img)
